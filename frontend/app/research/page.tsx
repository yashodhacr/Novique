  ...
  return (
    <div className="min-h-screen bg-[#050816] text-[#F8FAFC] relative font-sans selection:bg-[#6D5DF6]/30 selection:text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 flex flex-col gap-10 relative z-10 animate-fade-in">
        <div>
          <span className="text-[10px] font-extrabold uppercase tracking-widest text-[#14B8A6] mb-1.5 block">Academic Index</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredPapers.map((paper, idx) => (
            <div key={idx} className="bg-[#111827] border border-white/[0.05] p-7 md:p-8 rounded-3xl hover:border-[#6D5DF6]/30 hover:shadow-[0_12px_36px_rgba(109,93,246,0.06)] transition-all flex flex-col justify-between group">
              <div>
                <div className="flex items-center justify-between text-[10px] font-bold text-[#14B8A6] uppercase tracking-widest mb-4">
                  <span>{paper.confidence}</span>
                  <span className="text-zinc-500 font-semibold">{paper.citations}</span>
                </div>
                <h4 className="text-xl font-display font-extrabold text-white mb-1 group-hover:text-[#6D5DF6] transition-colors leading-snug">{paper.title}</h4>
              ...
              <div className="border-t border-white/[0.04] pt-5 mt-auto">
                ...
                <a
                  ...
                  className="w-full text-center py-2.5 bg-[#0B1220]/60 hover:bg-[#0B1220] border border-white/[0.05] hover:border-[#6D5DF6]/30 rounded-xl text-xs font-bold text-zinc-300 hover:text-white transition-all block"
                >
                  Read Paper &rarr;
                </a>
              </div>
